...
class VariantSelects extends HTMLElement {
  ...
  updateMasterId() {
    ...
    for(let removeFE of fieldsets){
      Array.from(removeFE.querySelectorAll('label')).find((radio) => {
    ...
    if(unavilable.length > 1){
      const toFindDuplicates = arry => arry.filter((item, index) => unavilable.indexOf(item) !== index)
      unavilable = toFindDuplicates(unavilable);
    }
    if(unavilable.length){
      const fieldsets = Array.from(this.querySelectorAll('fieldset'));
      const notaval = fieldsets.map((fieldset) => {
        return Array.from(fieldset.querySelectorAll('label')).find((radio) => {
          let result = radio.innerHTML.trim();
          if(unavilable.includes(result)){
              // radio.classList.add("noavaiable");
          }
        })
      });
    }
    this.chnageddata = this.options;
  }
  ...
  getProductData(){
  	this.productData = this.productData || JSON.parse(this.querySelector('#product_data').textContent);
